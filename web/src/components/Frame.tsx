...
const Frame = ({ photo, onClick }: FrameProps) => {
    ...
    return (
        <motion.div
            ...
            className="group relative w-full cursor-pointer overflow-hidden bg-transparent sm:rounded-sm"
        >
            {/* Minimal Skeleton */}
            {!isLoaded && !hasError && (
            ...
            <motion.img
                ...
                className="w-full h-auto block transition-transform duration-1000 ease-out group-hover:scale-110 sm:rounded-sm"
            />
            ...
            <div className="absolute top-4 left-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0 z-50">
                <span className="bg-black/80 backdrop-blur-2xl px-4 py-1.5 text-[8px] text-white/50 uppercase tracking-[0.4em] font-medium border border-white/5 sm:rounded-sm">
                    {photo.id} // Index
                </span>
            </div>

            {/* Hover Reveal Border - Desktop Only */}
            <div className="absolute inset-0 border border-white/0 group-hover:border-white/10 transition-all duration-500 pointer-events-none sm:rounded-sm" />

            {/* Soft Shadow on Hover - Desktop Only */}
